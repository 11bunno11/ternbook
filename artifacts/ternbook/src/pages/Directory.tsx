...
export default function Directory() {
  ...
  const {
    ...
    { query: { enabled: !activeSearch } as any },
  );
  ...
  return (
    <Layout>
      ...
      <div className="text-[0.65rem] text-muted-foreground mb-6 leading-relaxed">
        <span className="bg-[#181818] px-1 py-0.5 text-gray-400">tag:a,b</span>{" "}
        match tags &nbsp;
        <span className="bg-[#181818] px-1 py-0.5 text-gray-400">
          title:word
        </span>{" "}
        match name &nbsp;
        <span className="bg-[#181818] px-1 py-0.5 text-gray-400">
      ...
      <div className="flex flex-wrap items-end gap-3 mb-6">
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            ...
            onChange={(e) =>
              handleFilterChange("vibeWebCulture", e.target.value)
            }
            className="bg-input border border-border text-foreground text-xs px-2 py-1.5 min-w-[150px] appearance-none focus:outline-none focus:border-ring cursor-pointer"
          >
            <option value="">All</option>
            {tagsData?.vibeWebCulture?.map((t) => (
              <option key={t} value={t}>
                {t}
              </option>
            ))}
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            ...
            onChange={(e) =>
              handleFilterChange("contentSafety", e.target.value)
            }
            className="bg-input border border-border text-foreground text-xs px-2 py-1.5 min-w-[150px] appearance-none focus:outline-none focus:border-ring cursor-pointer"
          >
            <option value="">All</option>
            {tagsData?.contentSafety?.map((t) => (
              <option key={t} value={t}>
                {t}
              </option>
            ))}
      ...
      {!activeSearch && (
        <div
          ref={observerTarget}
          className="text-center text-[0.7rem] text-[#444] py-8 min-h-[64px]"
        >
          {isLoading
            ? "loading…"
            : !hasMore && displayedSites.length > 0
              ? "— end of directory —"
              : ""}
        </div>
      )}
